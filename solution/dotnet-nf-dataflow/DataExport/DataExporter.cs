...
namespace NF.Tools.DataFlow.DataExport
{
    ...
    {
        DataExporterOptions _exportOpt;
        CodeGeneratorOptions _codegenOpt;

        public DataExporter(DataExporterOptions opt)
        {
            this._exportOpt = opt;
            this._codegenOpt = new CodeGeneratorOptions
            {
                InputExcelDir = opt.InputExcelDir,
        ...
        public int Export()
        {
            var cg = new CodeGenerator(this._codegenOpt);
            if (!cg.TryGetCodeGenInfo(out CodeGenerator.CodeGenInfo codeGenInfo))
            {
                return 1;
            }
            ...
            using (SQLiteConnection conn = new SQLiteConnection(_exportOpt.OutputDatabasePath, _exportOpt.Password, SQLiteOpenFlags.ReadWrite | SQLiteOpenFlags.Create))
            {
